...
class shopListfeaturesPluginSettingsSetOptionsAction extends waViewAction
{
    public function execute()
    {
        ...
        $hash_settlements = shopListfeaturesPluginHelper::getHashSettlements('url');
        ...
        if (array_filter($features, wa_lambda('$id', 'return is_numeric($id);'))) {
        ...
        $set_options[] = array(
            'id'           => 'copy',
            'title'        => _wp('Copy settings'),
            'control_type' => waHtmlControl::CUSTOM.' shopListfeaturesPlugin::settingsCopySetOptions',
            'params' => array(
                'value' => $set_id,
            ),
            'class_name' => $set_id ? '' : 'hidden',
        );
