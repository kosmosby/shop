           ...
           //add edit link for checked feature checkboxes of existing set, except for 'Features'
            ...
            $('.listfeatures').on('click', '.add-set-link', function () {
                if (self.getSelectedSetId()) {
                    ...
                    $('.listfeatures input[type="text"], .listfeatures textarea').val('');
                    $('.listfeatures [name="template"]').val('default');
                    $('.listfeatures .set-options.copy').addClass('hidden');
                    $('.listfeatures .set-options.copy .set-options-copy-link').data('set-id', null);
                }
            });
                ...
                $.post(
                    ...
                    function (response) {
                        ...
                        if (response.data.new_set_id !== undefined) {
                            ...
                            var copy_control = form.find('.set-options.copy');
                            var copy_control_link = copy_control.find('.set-options-copy-link');
                            copy_control_link.data('set-id', new_set_id);
                            copy_control.removeClass('hidden');
                        }
            ...
            //set settings copying dialog
            $(document).on('click', '.listfeatures .set-options-copy-link', function () {
                var $link = $(this);

                $('<div><i class="icon16 loading"></i></div>').waDialog({
                    className: 'listfeatures-dialog',
                    onLoad: function () {
                        var $dialog = $(this);
                        var $dialog_content = $dialog.find('.loading').parent();
                        var $dialog_buttons = $dialog.find('.dialog-buttons-gradient');

                        $.get(
                            '?plugin=listfeatures&module=settings&action=controlCopySetOptionsDialog'
                                + '&set_id=' + $link.data('set-id')
                                + '&settlement_name=' + $('.listfeatures [name="settlement"]').children(':selected').text().trim()
                                + '&settlement_hash=' + $('.listfeatures [name="settlement"]').val(),
                            function (html) {
                                $response = $(html);
                                $dialog_content.replaceWith($response.find('.content').html());
                                $dialog_buttons.append($response.find('.buttons').html());
                            }
                        );
                    },
                    onSubmit: function ($dialog) {
                        ...
                        $.post('?plugin=listfeatures&module=settings&action=setOptionsCopy', $(this).serialize(), function (error) {
                            if (error.trim().length) {
                                $loading.addClass('hidden');
                                $dialog.hide();

                                $('<h1>' + $link.data('loc-error-title') + '</h1><p class="errormsg">' + error + '</p>').waDialog({
                                    height: '200px',
                                    width: '400px',
                                    buttons: '<a class="button blue cancel">' + $link.data('loc-error-ok') + '</a>',
                                    onClose: function () {
                                        $(this).remove();
                                        $dialog.show();
                                    }
                                ...
                                $loading.removeClass('loading').addClass('yes').animate({opacity: 0}, 700, function () {
                                    $dialog.trigger('close');
                                });
                            }
                        });
            ...
            //select app in set settings copying dialog
            $(document).on('click', '.listfeatures-dialog .set-options-copy-dialog-app', function () {
                var $link = $(this);
                var $links = $('.set-options-copy-dialog-app');
                var $li = $link.closest('li');
                var app = $link.data('app');
                var $settlements = $link.closest('.fields').find('[name="settlements[]"]');
                ...
                if (app) {
                    ...
                    $settlements.filter(function () {
                        return $(this).data('app') == app;
                    }).attr('checked', selected);
                } else {
                    if (selected) {
                        $li.closest('ul').children('li').addClass('selected');
                    } else {
                        $li.closest('ul').children('li').removeClass('selected');
                    }

                    $settlements.attr('checked', selected);
                }

                return false;
            });
            
            //unselect settlement in set settings copying dialog
            $(document).on('change', '.listfeatures-dialog .set-options-copy-dialog-settlement', function () {
                var $checkbox = $(this);
                
                if (!$checkbox.is(':checked')) {
                    $checkbox.closest('.field').find('.set-options-copy-dialog-app').filter(function () {
                        var $app_link = $(this);
                        return $app_link.data('app') == '0' || $app_link.data('app') == $checkbox.data('app');
                    }).closest('li').removeClass('selected');
                }
            });
            
            //select sets in set settings copying dialog
            $(document).on('click', '.listfeatures-dialog .set-options-copy-dialog-sets', function () {
                var $link = $(this);
                var $li = $link.closest('li');
                
                $li.toggleClass('selected');
                $link.closest('.field').find(':checkbox').attr('checked', $li.hasClass('selected'));
                
                return false;
            });
            
            //unselect set in set settings copying dialog
            $(document).on('change', '.listfeatures-dialog .set-options-copy-dialog-set', function () {
                var $checkbox = $(this);
                
                if (!$checkbox.is(':checked')) {
                    $checkbox.closest('.field').find('.set-options-copy-dialog-sets').closest('li').removeClass('selected');
                }
            });
        }
    };
