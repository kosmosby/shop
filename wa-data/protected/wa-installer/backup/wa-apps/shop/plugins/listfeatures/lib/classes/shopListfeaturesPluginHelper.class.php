...
class shopListfeaturesPluginHelper
{
    ...
    public static function getHashSettlements($field = null)
    {
        static $data;

        if (!$data) {
            try {
                $routing = include wa()->getConfig()->getPath('config', 'routing');
                ...
                foreach ($routing as $domain => $routes) {
                    ...
                    foreach ($routes as $route) {
                        ...
                        if (!in_array($route['app'], $apps)) {
                            continue;
                        }

                        $settlements[$domain_ids[$domain].'/'.$route['url']] = array(
                            'url' => $domain.'/'.$route['url'],
                            'name' => shopListfeaturesPluginHelper::settlementPunycodeToUtf($domain.'/'.$route['url']),
                            'app_id' => $route['app'],
                        );
                    }
                }
    ...
    public static function getSettlementConfig($settlement = null, $set_id = null, $field = null)
    {
        ...
        if ((!$plugin_config || empty($plugin_config['disable_new_route_check'])) && wa()->getEnv() == 'backend') {
    ...
    {
        $hash_settlements = self::getHashSettlements('url');
    ...
    public static function saveSettlementConfig($params)
    {
        ...
        if ($feature_ids === null) {
            //remove set from all settlements

            $update_all = true;
            foreach ($settlement_hashes as $hash) {
                if (isset($config[$hash][$set_id])) {
                    unset($config[$hash][$set_id]);
                }
            }
        } else {
            //save feature ids for new or specified set of specified settlement
            ...
            foreach ($settlement_hashes as $hash) {
                ...
                while (++$set_id_counter <= $set_id) {
                    if (!isset($config[$hash][$set_id_counter]['features'])) {
                        $config[$hash][$set_id_counter]['features'] = array();
                    }
                }

                if ($hash == $settlement) {
                    //save feature set for selected settlement only

                    //remove unselected features from config
                    foreach ($config[$hash][$set_id]['features'] as $feature_id => $feature_config) {
                        if (!in_array($feature_id, $feature_ids)) {
                            unset($config[$hash][$set_id]['features'][$feature_id]);
                        }
                    }

                    //add newly selected features to config while keeping existing ones unchanged
                    foreach ($feature_ids as $feature_id) {
                        if (!in_array($feature_id, array_keys($config[$hash][$set_id]['features']))) {
                            $config[$hash][$set_id]['features'][$feature_id] = array();
                        }
                    }
                }
            }
